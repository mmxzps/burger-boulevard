    ...
    fetchBurgers() {
      ...
      api.getComponents(1, 1)
        ...
        .catch(error => {
          ...
          this.error = "Kunde inte hämta hamburgare";
          this.loading = false;
        });
    }
  }
...
<template>
  <div>
    ...
    <h1 class="meny-h1">Hamburgare</h1>

    <div v-if="loading" class="loading-state">Laddar hamburgare...</div>
    <div v-else-if="error" class="error-state">{{ error }}</div>
    <ul v-else class="cards-container">
      <li class="menu-card" v-for="menu in menus" :key="menu.id">
        <router-link :to="`/burger/${menu.id}`" class="menu-link">
          <h2>{{ menu.name }}</h2>
          <img src="../categoryImg/baraBurgar.png" alt="Bild saknas" class="burgerPic" />
